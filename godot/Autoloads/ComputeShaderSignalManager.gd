...
func register_perspective_tilt_mask_fragment(_perspective_tilt_mask_fragment: PerspectiveTiltMaskFragment) -> void:
    if self.perspective_tilt_mask_fragment and self.visual_illusion_updated.is_connected(_on_visual_illusion_fragment_updated):
        self.visual_illusion_updated.disconnect(_on_visual_illusion_fragment_updated)
    perspective_tilt_mask_fragment = _perspective_tilt_mask_fragment
    visual_illusion_updated.connect(_on_visual_illusion_fragment_updated)


func _on_visual_illusion_fragment_updated(sprite_index, center_px, half_size_px, altitude_normal, ascending) -> void:
...
func register_umbral_shadow_fragment(_umbral_shadow: ShaderMaterial) -> void:
    ...
    if perspective_tilt_mask_fragment:
        umbral_shadow.set_shader_parameter(
            "iChannel1",
            perspective_tilt_mask_fragment.get_perspective_tilt_mask_texture_fragment()
        )
