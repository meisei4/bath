...
const SINGLE_FRAME_AVOIDANCE_BUFFER: float = 1.0 / 60.0
...
func _initialize_particle_material() -> void:
    ...
    self.emission_rect_extents = get_viewport().size / 2.0 #TODO: ???? WHAT? EXTENTS IS A kind of radius for a rectangle, not the actual WxH, half of that
