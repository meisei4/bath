...
func setup_glacier() -> void:
    ...
    glacier_mass_distribution.initialize_mass_distribution(
        GLACIER_MAP_WIDTH, GLACIER_MAP_HEIGHT, glacier_map
    )
...
func _on_hydrofracture_cycle() -> void:
    var fractures_started = hydrofracture_manager.run_cycle(
        glacier_mass_distribution, glacier_map, fractures_per_cycle
    )
    if fractures_started == 0:
        return

    var new_icebergs = iceberg_manager.identify_and_create_icebergs(
        glacier_mass_distribution, glacier_map
    )
...
func _draw() -> void:
    ...
    for line in lines:
        ...
        draw_line(
            ...
            hydrofracture_manager.fracture_renderer.line_color,
            hydrofracture_manager.fracture_renderer.line_width
        )
