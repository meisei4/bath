...
func run_cycle(
    glacier_mass_distribution: GlacierMassDistribution,
    glacier_map: TileMapLayer,
    fractures_per_cycle: int
) -> int:
    ...
    var selected_fracture_starts = fracture_zones.slice(
        0, min(fractures_per_cycle, fracture_zones.size())
    )
...
func find_fracture_zones(
    glacier_mass_distribution: GlacierMassDistribution, glacier_map: TileMapLayer
) -> Array[Vector2i]:
...
func propagate_fracture(
    mass_distribution: GlacierMassDistribution,
    glacier_map: TileMapLayer,
    position: Vector2,
    current_depth: int = 0
) -> void:
    ...
    for cell in surrounding_cells:
        if (
            randf() < fracturing_coefficient
            and mass_distribution.get_state(cell) == GlacierCellState.STATE.INTACT
        ):
