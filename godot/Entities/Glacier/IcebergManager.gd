...
@export var max_icebergs_to_form_per_cycle: int = 6
@export var max_iceberg_clusters_to_move_per_cycle: int = 4
...
func iceberg_formation_phase(glacier_data: GlacierData, fractured_cells: Array[Vector2i]) -> int:
    ...
    for y in range(glacier_dimensions.y):
        for x in range(glacier_dimensions.x):
            ...
            if glacier_data.get_state(current_cell_position) == GlacierCellState.STATE.FRACTURED:
...
func move_icebergs(glacier_data: GlacierData) -> void:
    ...
    for y in range(glacier_dimensions.y - 1, -1, -1):
        for x in range(glacier_dimensions.x):
            ...
            if visited.has(current_cell_position):
                continue
            if glacier_data.get_state(current_cell_position) == GlacierCellState.STATE.ICEBERG:
                ...
                if number_of_clusters_moved < max_iceberg_clusters_to_move_per_cycle:
...
func notify_cell_fractured(cell_position: Vector2i, glacier_data: GlacierData) -> void:
    if active_moving_iceberg_cluster.size() == 0:
        return

    if glacier_data.get_state(cell_position) == GlacierCellState.STATE.FRACTURED:
        unify_cluster_with_cell(glacier_data, active_moving_iceberg_cluster, cell_position)
    else:
        print("[IcebergManager] notify_cell_fractured called, but cell not FRACTURED? State:",
            glacier_data.get_state(cell_position))
...
func unify_cluster_with_cell(
    ...
    var connected_cells: Array[Vector2i] = GlacierUtil.flood_fill(
        ...
        start_cell,
        func(pos: Vector2i) -> bool:
            var state: int = glacier_data.get_state(pos)
            return (state == GlacierCellState.STATE.FRACTURED or state == GlacierCellState.STATE.ICEBERG)
    )
