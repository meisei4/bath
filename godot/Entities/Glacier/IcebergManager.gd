...
func identify_and_create_icebergs(
	mass_distribution: GlacierMassDistribution, glacier_map: TileMapLayer
) -> int:
	...
	for y in range(mass_distribution.mass_distribution.size()):
		for x in range(mass_distribution.mass_distribution[y].size()):
			...
			var mass: int = perform_mass_flood_fill(
				mass_distribution, glacier_map, position, visited_cells
			)
...
func perform_mass_flood_fill(
	...
	while stack:
		...
		for adjacent_position in surrounding_cells:
			if (
				not visited.has(adjacent_position)
				and (
					mass_distribution.get_state(adjacent_position)
...
func create_iceberg(
	mass_distribution: GlacierMassDistribution, position: Vector2i, glacier_map: TileMapLayer
) -> void:
