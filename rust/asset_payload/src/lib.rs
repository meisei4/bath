...
macro_rules! define_assets {
    (binary: { $( $bin_ident:ident => $bin_path:expr ),* $(,)? },
    string: { $( $str_ident:ident => $str_path:expr ),* $(,)? }) => {
        pub struct ResourcePaths;
        impl ResourcePaths {
            $( pub const $bin_ident: &'static str = concat!("res://assets/", $bin_path); )*
            $( pub const $str_ident: &'static str = concat!("res://assets/", $str_path); )*
        }

        pub mod runtime_io {
            pub const ROOT: &str = "../assets";
            $( pub const $bin_ident: &'static str = concat!("../assets/", $bin_path); )*
            $( pub const $str_ident: &'static str = concat!("../assets/", $str_path); )*
        }

        #[cfg(feature = "nasa-embed")]
        pub mod embed {
            pub const ROOT: &str = "../../../assets";
            $( pub const $bin_ident: &'static [u8] = include_bytes!(concat!( "../../../assets/", $bin_path)); )*
            $( pub const $str_ident: &'static str   = include_str!(concat!( "../../../assets/", $str_path));  )*
        }
    }
}
...
define_assets! {
    binary: {
        CACHED_WAV_PATH               => "audio/cached_wav.wav",
