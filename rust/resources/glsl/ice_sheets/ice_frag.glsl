...
void main() {
    ...
    vec3 terrain = solid ? vec3(SOLID_REGION_BRIGHTNESS) : pow(WATER_COLOR, vec3(float(depth) / WATER_DEPTH_DIV));

    vec3 water = solid ? terrain : (depth > WATER_STATIC_TH ? terrain * WATER_DARKEN_MULT : terrain);
