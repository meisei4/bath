...
/// On desktop (any target ≠ wasm32), this is the real Aubio‐based function.
/// On wasm32, it never gets compiled. Instead, the stub below is used.
...
// const I16_TO_F32: f32 = 1.0 / (i16::MAX as f32);
// use beat_detector::{StrategyKind, Strategy};
// pub fn detect_bpm_from_beat_detector(wav_bytes: &[u8]) -> f32 {
//     let mut reader = match WavReader::new(Cursor::new(wav_bytes)) {
//         Ok(r) => r,
//         Err(e) => {
//             godot_print!(
//                 "detect_bpm_from_bytes: failed to parse WAV bytes: {}",
//                 e
//             );
//             return 0.0;
//         }
...
//     let mut detector: Box<dyn Strategy + Send> =
//         StrategyKind::Spectrum.detector(spec.sample_rate);
...
//         let window: &[i16] = &mono_samples[start..end];
