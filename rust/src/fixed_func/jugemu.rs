...
pub fn draw_near_plane_projection_markers_for_model(
    rl3d: &mut impl RaylibDraw3D,
    observer: &Camera3D,
    near_clip_plane: f32,
    ...
    for local in local_points.iter() {
        ...
        if let Some(intersection_point) =
            compute_near_plane_intersection_point_for_world_point(observer, near_clip_plane, world_point)
        {
            unsafe { rlSetLineWidth(1.0) }
...
pub fn compute_near_plane_intersection_point_for_world_point(
    observer: &Camera3D,
    near_clip_plane: f32,
    world_point: Vector3,
) -> Option<Vector3> {
...
/// Exact pixel mapping for a world point as seen by `observer`.
/// Returns (pixel_x, pixel_y) in screen coordinates if the point projects inside the viewport.
pub fn project_world_point_to_main_observer_pixels(
    observer: &Camera3D,
    aspect: f32,
    near_clip_plane: f32,
