...
//TODO]: same as deform_mesh_by_silhouette_radii but for slices
pub fn deform_vertices_by_silhouette_radii(vertices: &mut [Vector3], radial_magnitudes: &[f32]) {
    for vertex in vertices {
        let angle = vertex.y.atan2(vertex.x).rem_euclid(TAU);
        let sample_position = angle / TAU * RADIAL_SAMPLE_COUNT as f32;
        let lower_index = sample_position.floor() as usize % RADIAL_SAMPLE_COUNT;
        let upper_index = (lower_index + 1) % RADIAL_SAMPLE_COUNT;
        let upper_weight = sample_position.fract();
        let lower_weight = 1.0 - upper_weight;
        let equivalent_radius =
            radial_magnitudes[lower_index] * lower_weight + radial_magnitudes[upper_index] * upper_weight;
        vertex.x *= equivalent_radius;
        vertex.y *= equivalent_radius;
        vertex.z *= equivalent_radius;
    }
}

pub fn bake_samples_for_single_mesh_no_rotation(
    delta_time: f32,
    sample_count: usize,
    output_width: i32,
    output_height: i32,
    ...
    let base_model = renderer.handle.load_model(&renderer.thread, SPHERE_PATH).unwrap();
    let base_vertices = base_model.meshes()[0].vertices();
    let mut vertex_sets: Vec<Vec<Vector3>> = Vec::with_capacity(sample_count);
    for sample_index in 0..sample_count {
        let time = sample_index as f32 * delta_time;
        let camera_yaw_rad = -time * ANGULAR_VELOCITY;
        let mut rotated: Vec<Vector3> = base_vertices.to_vec();
        for vertex in &mut rotated {
            let (x0, z0) = (vertex.x, vertex.z);
            vertex.x = x0 * camera_yaw_rad.cos() + z0 * camera_yaw_rad.sin();
            vertex.z = -x0 * camera_yaw_rad.sin() + z0 * camera_yaw_rad.cos();
        }
        let silhouette_img = generate_silhouette_image(output_width, output_height, time);
        let radial_magnitudes = build_radial_magnitudes(&silhouette_img);
        deform_vertices_by_silhouette_radii(&mut rotated, &radial_magnitudes);
        let inverse_yaw = -camera_yaw_rad;
        for vertex in &mut rotated {
            let (x0, z0) = (vertex.x, vertex.z);
            vertex.x = x0 * inverse_yaw.cos() + z0 * inverse_yaw.sin();
            vertex.z = -x0 * inverse_yaw.sin() + z0 * inverse_yaw.cos();
        }
        vertex_sets.push(rotated);
    }
    vertex_sets
}

pub fn blend_into_mesh(
    destination_mesh: &mut WeakMesh,
    vertex_atlas: &[Vec<Vector3>],
    elapsed_time: f32,
    delta_time: f32,
) {
    let duration = vertex_atlas.len() as f32 * delta_time;
    let time = elapsed_time % duration;
    let frame = time / delta_time;
    let current_frame = frame.floor() as usize % vertex_atlas.len();
    let next_frame = (current_frame + 1) % vertex_atlas.len();
    let interpolation_weight = frame.fract();
    let dst_vertices = destination_mesh.vertices_mut();
    for ((dst_vertex, src_vertex), next_vertex) in dst_vertices
        .iter_mut()
        .zip(vertex_atlas[current_frame].iter())
        .zip(vertex_atlas[next_frame].iter())
    {
        dst_vertex.x = src_vertex.x * (1.0 - interpolation_weight) + next_vertex.x * interpolation_weight;
        dst_vertex.y = src_vertex.y * (1.0 - interpolation_weight) + next_vertex.y * interpolation_weight;
        dst_vertex.z = src_vertex.z * (1.0 - interpolation_weight) + next_vertex.z * interpolation_weight;
    }
}
